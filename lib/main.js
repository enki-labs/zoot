...
Zoot.prototype.addTask = function (taskData, searchTags, addTags, removeTags, markDirty, callback)
{
    ...
    if (searchTags == null || searchTags.length == 0) //no parent
    {
        ...
        self.neoClient.cypherQuery(query, function(err, existingNodes) {
            ...
            {
                this.key = uuid.v1();
                this.redisClient.set("task:" + this.key, JSON.stringify(taskData), function (err) { 
                    if (err) throw err;

                    //NB!! Query must be space rather than empty string or Neo4j errors silently.
                    var nodeInfo = {'dirty':markDirty, 'tag':addTags.sort(), 'key':this.key, 'query':' '};
            ...
            for (index in result.data)
            {
                ...
                self.redisClient.get("task:" + taskNode.key, function (err, parentData) {
                    ...
                    self.neoClient.cypherQuery(this.findExistingChildren, function(err, existingChildren) {
                        ...
                        {
                            this.key = uuid.v1();
                            self.redisClient.set("task:" + this.key, JSON.stringify(taskData), function (err, data) {

                                if (err) throw err;

                                self.neoClient.insertNode({dirty: markDirty, tag: this.newTags, key: this.key, query: query }, function (err, newNode) {
